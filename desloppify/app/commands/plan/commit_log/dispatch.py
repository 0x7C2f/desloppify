"""CLI handlers for `plan commit-log` grouped by commit-log capability."""
...
def _maybe_update_pr_body(plan: dict) -> None:
    ...
    try:
        ...
        if ok:
            ...
            print(
                colorize(
                    f"  Could not update PR #{pr_number} (gh may not be available).",
                    "yellow",
                )
            )
...
def cmd_commit_log_dispatch(args: argparse.Namespace) -> None:
    ...
    if not config.get("commit_tracking_enabled", True):
        print(
            colorize(
                "  Commit tracking is disabled. Enable with: desloppify config set commit_tracking_enabled true",
                "yellow",
            )
        )
        return
